...
def load_singles_hist(filename='singles_hist.npz',filepath=None):
    """
...
def coarsen_bhm(bicorr_hist_master,dt_bin_edges,C):
    ...
    for bin1 in np.arange(0,bicorr_hist_master_coarse.shape[2]):
        for bin2 in np.arange(0,bicorr_hist_master_coarse.shape[3]):
            bicorr_hist_master_coarse[:,:,bin1,bin2] = np.sum(bicorr_hist_master[:,:,C*bin1:C*(bin1+1),C*bin2:C*(bin2+1)],axis=(2,3))
    
    dt_bin_edges_coarse = dt_bin_edges[0::C]
    
    return bicorr_hist_master_coarse, dt_bin_edges_coarse
...
def generate_pair_is(det_df, th_min = None, th_max = None, i_bin = None):
    """
...
def build_bhp(bhm, dt_bin_edges, num_fissions = None,
    ...
    norm_factor : float
        Normalization factor. Counts per fission = 
    """
    if print_flag:
